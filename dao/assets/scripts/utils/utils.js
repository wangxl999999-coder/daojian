...
var util = {
    ...
    isInSameDay (timeStamp) {
        ...
        timeStamp *= 1000;
        let regTime = new Date(timeStamp);
        let regYear = regTime.getFullYear();
        let regMonth = regTime.getMonth();
        let regDay = regTime.getDate();
    
        return nowYear === regYear && nowMonth === regMonth && nowDay === regDay;
    },

    checkDays (timeStamp1, timeStamp2) {
        let date1 = new Date(timeStamp1);
        let date1Year = date1.getFullYear();
        let date1Month = date1.getMonth();
        let date1Day = date1.getDate();
        // let date1Week = date1.get
	    ...
	    var old_count =parseInt(timeStamp1/oneDayTime);
	    var now_other =parseInt(timeStamp2/oneDayTime);
        let isSameWeek = parseInt((old_count+4)/7) == parseInt((now_other+4)/7);

        let isSameDay = date1Year === date2Year && date1Month === date2Month && date1Day === date2Day;
        

        return {sameDay: isSameDay, sameWeek: isSameWeek};
    },
    ...
    cutString (render) {
        ...
        for (var i = 0, n = strArr.length; i < n; i++) {
            ...
            result += strArr.charAt(i);
            if (len >= 14)   // 最多14个英文，7个汉字
            {
                return result;
            }
        }

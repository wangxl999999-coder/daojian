...
cc.Class({
    ...
    _allDirectionsMove: function(dt) {
        ...
        if (!PlayerManager.player.node.active) {
            return;
        }
        if (this._joyCom.ring._speed === 0) {
            return;
        }
        if (!PlayerManager.player.isDefence) {
            this._speed = PlayerManager.player.speed;
            if (this._angle !== null && this._angle !== undefined) {
                let deltaX = Math.cos(this._angle * (Math.PI / 180)) * this._speed;
                let deltaY = Math.sin(this._angle * (Math.PI / 180)) * this._speed;
                this._playerNode.x += deltaX * dt;
                this._playerNode.y += deltaY * dt;
            }
        }
    },
